...
# =============================================================================
    ...
    **Как изменить настройки:**
    ...
    with col1:
        with st.expander("🔓 Разблокировать базу"):
            st.info("""
            Если вы получаете ошибку "database is locked", 
            нажмите эту кнопку для сброса соединений.
            """)
            if st.button("🔓 Сбросить соединения", type="secondary"):
                from src.db.database import close_all_connections
                close_all_connections()
                st.success("Соединения сброшены. Перезагрузите страницу.")
                st.rerun()
    
    with col2:
        with st.expander("🗑️ Очистить базу данных"):
