    ...
    **Важно:** Владельцы с малым числом закрытых позиций фильтруются для точности.
    ...
    if stats['total_positions'] == 0:
        ...
        if st.button("📊 Показать рейтинг", type="primary"):
            ...
            owners = analyzer.get_top_owners(
                limit=limit * 2,  # Запросим больше, потом отфильтруем
                order_by=order_by,
                min_positions=1,  # Базовый фильтр
            )
            
            # Фильтруем по минимальному числу закрытых позиций
            owners = [o for o in owners if o.closed_positions >= min_closed][:limit]
